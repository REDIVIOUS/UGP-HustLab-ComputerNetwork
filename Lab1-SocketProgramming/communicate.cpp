...
Communicate::Communicate(QObject *parent)
    :QThread(parent)
{
    qDebug()<<"recieve id: "<<QThread::currentThreadId();
}
...
void Communicate::run()
{
    while(!isInterruptionRequested())
    {
        ...
        //接受请求报文
        int rtn; //接受的字节数
        rtn=recv(acpt,recvBuf,sizeof(recvBuf),0); //按照实验手册上的设置配置recv函数
        ...
        else if(filetype(URL,".jpeg")||filetype(URL,".png")||filetype(URL,".jpg"))
        {
            content_type="Content-Type: image/"+URL.substr(URL.rfind('.')+1)+"\r\n";
        }
        else if(filetype(URL,".ico"))
        {
        ...
        /*响应过程*/
        //发送响应报文
        rtn=send(acpt,respondhead.c_str(),(int)(respondhead.length()),0); //按照试验手册上进行
        //发送请求的文件
        int bufReadNum;
        while(true)
        {
            ...
            if((send(acpt,sendBuf,bufReadNum,0))==SOCKET_ERROR) //发送失败
            {
                rtn=SOCKET_ERROR;
                break;
            }
        ...
        msg.clear();
        msg<<QString::fromStdString(clientaddr)<<QString::fromStdString(to_string(clientport))<<QString::fromStdString(URL.substr(1));
        if(rtn==SOCKET_ERROR)
        {
